...
@interface TMShopCarViewController ()
{
  float _price;
}
@end

@implementation TMShopCarViewController
...
//    _marrayProductid=[[NSMutableArray alloc]init];
//    arrayLabel=[[NSMutableArray alloc]init];
//    arrayTextFeild=[[NSMutableArray alloc]init];
...
-(UIView*)initNavigationBar
{
    ...
    if (_isTabbar==YES)
    {
        ...
        btnDD.font=[UIFont systemFontOfSize:16.0f];
...
//    [checkBtn1 setImage:BundleImage(@"ic_01_n_.png") forState:0];
    ...
    UILabel *price_label=[[UILabel alloc]initWithFrame:CGRectMake(title_label.frame.size.width+title_label.frame.origin.x, 15, 75, 20)];
    price_label.tag=-101;
...
//        
//        
//        UIImageView *image1=(UIImageView*)[self.view viewWithTag:[path row]+100];
//        image1.image= BundleImage(@"ic_01_h_.png") ;
//        
//    
//    }
...
//{
//    if ([son.getMethod isEqualToString:@"FWShopCarList"])
//    {
//        if ([son getError]!=0) {
//            return;
//        }
//        Msg_FW_ShopCarts_Builder*  checkOut=(Msg_FW_ShopCarts_Builder*)[son getBuild];
//        [_marrayAll removeAllObjects];
//        [_marrayAll addObjectsFromArray:checkOut.shopCartsList];
//        [_checkBtnArray removeAllObjects];
//        [_tableView reloadData];
//        
//        amoutLabel.text=[NSString stringWithFormat:@"%d",[_marrayAll count]];
//    }
//    
//    if ([son.getMethod isEqualToString:@"FWShopCartDelete"])
//    {
//        if ([son getError]==0) {
//            NSMutableIndexSet *indexSet = [NSMutableIndexSet indexSet];
...
//            {
//                [indexSet addIndex:[path row]];//添加指定单元行的下标
//                [tempArray addObject:[_marrayAll objectAtIndex:[path row]]];//
//                productids=[NSString stringWithFormat:@"%@,",[[[[_marrayAll objectAtIndex:[path row]] productList] objectAtIndex:0] id]];
//                _strings=[_strings stringByAppendingString:productids];
//            }
//            
//            [_marrayAll removeObjectsAtIndexes:indexSet];
//            [_checkBtnArray removeObjectsAtIndexes:indexSet];
//            
//            [_tableView deleteRowsAtIndexPaths:deleteArray withRowAnimation:NO];
//            
//            [deleteArray removeAllObjects];//批量删除
//            _isSelecedAll = NO;
//            amoutLabel.text = [NSString stringWithFormat:@"%d",[_marrayAll count]];
//            [_tableView reloadData];
...
-(void)btnSelecdAll:(id)sender
{
    ...
    if (_isSelecedAll==NO)
    {
        ...
        UILabel *labelPriceSum=(UILabel*)[self.view viewWithTag:-101];
        labelPriceSum.text=[NSString stringWithFormat:@"%.2f", [[_arrayPrice objectAtIndex:btn.tag] floatValue]*[field.text floatValue]];;
        _isSelecedAll=YES;
        _isPOP=YES;
    }else{
...
-(void)btnAdd:(id)sender
{
    ...
    if(field.text.intValue<10)
    {
        field.text=[NSString stringWithFormat:@"%d",[field.text intValue]+1];
    }else if(field.text.intValue==10)
    {

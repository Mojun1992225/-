...
- (void)buildContent
{
    [self addLabel:CGRectMake(20, 85, 265, 16) view:self.view font:16 text:@"联系方式（Email或者QQ)" backgroudcolor:[UIColor clearColor] textcolor:[UIColor blackColor]];
    
    tf_LinkWay = [self addTextField:CGRectMake(20, 115, 275, 45) withPlaceholder:nil withview:self.view withtag:21110];
    [self.view addSubview:tf_LinkWay];
    
    [self addLabel:CGRectMake(20, 175, 265, 16) view:self.view font:16 text:@"反馈内容" backgroudcolor:[UIColor clearColor] textcolor:[UIColor blackColor]];
    
    tv_FeedBackContent = [self addTextView:CGRectMake(20, 205, 275, 120) view:self.view];
    [self.view addSubview:tv_FeedBackContent];
    
    
    UIButton * submitFeedBack = [UIButton buttonWithType:UIButtonTypeCustom];
    [submitFeedBack setFrame:CGRectMake(20, 335, 93, 34)];
    ...
    UILabel *tishiLabel = [[UILabel alloc] initWithFrame:CGRectMake(245, 330, 70, 34)];
    tishiLabel.font = [UIFont systemFontOfSize:14.0f];
    tishiLabel.textColor = [UIColor colorWithRed:0.58 green:0.58 blue:0.58 alpha:1];
    tishiLabel.text = @"限200字";
    [self.view addSubview:tishiLabel];
    
    CGRect frame=[UIScreen mainScreen].bounds;
    UIButton *sure=[[UIButton alloc]initWithFrame:CGRectMake(20, 380, frame.size.width-40, 45)];
    ...
    [sure addTarget:self action:@selector(tijiao) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:sure];
}

#pragma mark - 用户反馈提交按钮
-(void)tijiao
{
